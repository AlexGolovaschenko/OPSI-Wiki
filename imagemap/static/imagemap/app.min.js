        ...
        d = {
            ...
            asHTML: function(b) {
                ...
                a.each(c[b].area, function(d, e) {
                    ...
                    var i = a("<a />");
                    i.attr("target", c[b].area[d].target), i.attr("alt", c[b].area[d].title), i.attr("title", c[b].area[d].title), i.attr("href", c[b].area[d].href), i.attr("coords", f.join(",")), g.push(i[0].outerHTML)
                }), f.append("\n    " + g.join("\n    ") + "\n");
                var h = "<!-- Image Map Generated by http://www.image-map.net/ -->";
                return h + "\n" + d[0].outerHTML + "\n\n" + f[0].outerHTML
            }, */
            asHTML: function(b) {
                var e = c[b].options.src;
                var d = a("<img />");
                var img = new Image;
                var img_width = 100, img_height = 100;
                img.src = c[b].options.src;
                if (img.src != undefined) { 
                    img_width = img.naturalWidth; 
                    img_height = img.naturalHeight;
                    c[b].options.file.length > 0 && (e = c[b].options.file), d.attr("src", e), d.attr("style", `position:absolute; width:${img_width}px; max-width:${img_width}px;`); 
                };

                var div = a("<div />");
                div.attr("class", "imgRegion"), div.attr("style", `height:${img_height + 35}px; max-width:${img_width}px;`);
                div.append("\n  " + d[0].outerHTML);
                ...
                a.each(c[b].area, function(d, e) {
                    var f = [];
                    a.each(e.coords, function(a, b) {
                            f.push(b.naturalX, b.naturalY)
                        }) 
                    var h = []
                    h.push(f[2] - f[0], f[3] - f[1]);
                    f = f.slice(0, 2);
                    f = f.concat(h);

                    counter ++;
                    styles += `    .region${counter} {left:${f[0]}px; top:${f[1]}px; width:${f[2]}px; height:${f[3]}px;}\n`;

                    var i = a("<a />");
                    i.attr("target", c[b].area[d].target), i.attr("title", c[b].area[d].title), i.attr("href", c[b].area[d].href), i.attr("class", `region region${counter}`), g.push(i[0].outerHTML)
                }), div.append("\n  " + g.join("\n  ") + "\n");

                var header = '', footer = '';
                header = "<html>\n<head>\n  <style>\n";
                header += "    div.imgRegion {position: relative; overflow-x: scroll; overflow-y: hidden;}\n";
                if ( $("#use_radius").val() == 'True' ) {
  				  header += "    .region {position: absolute; border-radius:10px;}\n";
                } else {
                  header += "    .region {position: absolute;}\n";    
                }
  				header += "    .region:hover {box-shadow: inset 0 0 300px 0 rgba(0, 0, 0, 0.4);}\n";
  				header += "\n";
                header += styles;
                header += "  </style>\n</head>\n";
                footer = "</html>";
